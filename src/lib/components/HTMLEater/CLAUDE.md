...
- `synth.js`: Tone.js engine, pitch mapping, instruments, scheduling, setters, `defaults`, `presets`
...
- **Settings flow one way.** Component `$state`, then one `$effect` per setting, then `synth.set*()`. Setters clamp, ignore invalid input (empty number fields give `null`), and apply to Tone once loaded. Defaults live only in `synth.defaults`, and presets only in `synth.presets`; a preset is applied by writing the component `$state`, so it takes the same path as a knob.
...
- `.container::before` washes the surface with `--primary-hover`, `contrast(2) invert(1)` and `mix-blend-mode: overlay` at 5% — a flat tint, no image. It's `z-index: -1` inside an `isolation: isolate` container, so it sits above the container's background but under the content, and the tabs' opaque `--bg` covers it: the wash only ever shows on the chrome around the windows. `CyberPlaque`'s plate takes `--surface` and the same wash (in `.plate::after`, under the ring and lettering), so the plaques read as the same material as the chrome. Gilt stays on the ring, the callouts and the glow — on a plate it would be `--primary-hover` mixed into `--bg`, which is a different color from `--surface` and swamps the wash.
...
- The preset select opens the settings panel, right under the heading and above the sliders: `label.preset` in the scale selects' muted lettering, but with the select on the same line as the word (it has to outrank `.settings label`'s `display: contents`, which is for the panel's two-column rows). Each entry in `synth.presets` patches only the settings it names (speed, tonal center, drum volume and the voice envelope), leaving volume, scale and the shelf where the panel left them; `punctual` is `defaults`, so it's the way back. `presetName` is `$derived` by comparing the panel against each preset, the same way the scale select earns its name, and the `custom` option is only listed while it applies. The comparison is exact equality, which holds because `Knob` snaps to its step and drops float noise — a preset value off the step grid would never match itself.
- An uploaded file is cut to `MAX_SOURCE_CHARS` (10,000) in `onPick`, before it becomes `loadedText`. A whole page of markup is hours of steps and re-highlights the panel on every one. The cut is silent: the source row is already title, name and controls on one line, and a note about it ran into them. The bundled sample is well under the cap.
...
- `.knobs` is a fixed two-column grid, so Speed and Tonal center sit side by side and the four voice knobs make a 2x2. It used to be a wrapping flex row, but a knob is only as wide as its widest part: a readout going from three digits to two narrowed it, slid the row along and could wrap one knob onto a second line mid-drag. Each knob centers itself in its half instead, so the dials never move.
