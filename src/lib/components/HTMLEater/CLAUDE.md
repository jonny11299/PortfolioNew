...
- `.container::before` lays `screen.webp` over the surface at 70%, grayscaled, inverted and `mix-blend-mode: overlay`, so only the image's light and dark carry over, flipped and the surface keeps its hue. It's `z-index: -1` inside an `isolation: isolate` container, so it sits above the container's background but under the content, and the tabs' opaque `--bg` covers it: the texture only ever shows on the chrome around the windows. `screen.webp` is the grayscale, losslessly-compressed version of the 1.7 MB `screen.png` it was dropped in as — 275 KB, and identical once the CSS grayscales it. The PNG is no longer referenced, so nothing emits it into the build.
